...
unsigned int sound_speed = 11025;    /* sample rate */

int sound_debug = 0;            /* debug mode */
int sound_feedback = 0;         /* -1, running out of sound
                                   +0, lots of sound, do something */
unsigned int sound_minfields = 2;       /* min fields to try to buffer */
...
sint32 sound_soundbuf[2][1024];
sint16 end_soundbuf[2][1024];
...
int sound_init(void)
{
  ...
  sound_sampsperfield = sound_speed / fps;
  sound_normalsamples = 48000 / fps;
...
void sound_startfield(void)
{
	...
	memset(&sound_soundbuf[0], 0, sound_sampsperfield<<2);
	memset(&sound_soundbuf[1], 0, sound_sampsperfield<<2);
}
